...
<h1 class="text-center text-capitalize text-white">List of appointment.....</h1>
<hr />
<form action="delete_user_appointment_request.php" method="POST">
        ...
        while ($drow = mysqli_fetch_array($edit_run_doctor_profile_query)) {
            ...
            <tr>
                <th>
                    <h3>
                        <?php
                        echo "<a href='detail.php?s_id={$drow['s_id']}'>{$drow['dfname']}{$drow['dlname']}</a>";
                        ?>&nbsp; &nbsp;Specialist Of &nbsp;&nbsp;  <?php echo $drow['specialist'];  	?>
                    </h3>
                </th>
                <th>
                    <h3>
                        <?php echo $drow['booking_date'];  	?>
                    </h3>
                </th>
                <th>
                    <h3>
                        <?php echo $drow['booking_time'];  	?>
                    </h3>
                </th>
                <th>
                    <h3>
                        <?php
                        echo  "<a href='user_detail.php?id={$drow['id']}'>{$drow['ufname']}{$drow['ulname']}</a>";
                        ?>
                    </h3>
                </th>
            </tr>
    ...
    <div class="container">
        <ul class="pager font-weight-bold text-monospace active ">
            <li class="previous bg-dark ">
                <a href="view_admin_home_page.php">Previous</a>
            </li>
        </ul>
    </div>
</form>
